-- MVP Make: anti-reuso de comprobante + RPC para validar orden y emitir tickets (idempotente)
-- Ejecutar en Supabase SQL Editor (proyecto remoto) después de schema + policies + ticketera-public.

-- -----------------------------------------------------------------------------
-- 1) orders.proof_sha256 (hash del archivo del comprobante)
-- -----------------------------------------------------------------------------
...
comment on column public.orders.proof_sha256 is 'SHA-256 hex del archivo de comprobante; único para evitar re-subir la misma imagen en otra orden.';

-- -----------------------------------------------------------------------------
-- 2) RPC: finalizar pago y emitir tickets (idempotente)
-- -----------------------------------------------------------------------------
...
begin
  ...
  if v_existing >= v_needed and v_needed > 0 then
  ...
  loop
    ...
    v_uid := encode(gen_random_bytes(18), 'hex');
...
comment on function public.finalize_order_payment is 'Emite tickets para una orden pending_validation/manual_review de forma idempotente; usar desde Make (service_role) o server actions con rol adecuado.';
